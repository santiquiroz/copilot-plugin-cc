...
- Use exactly one `Bash` call: `copilot -p "<task>" -s --no-ask-user --max-ai-credits 10 --allow-tool='shell(git:*)' --allow-tool=write --deny-tool='shell(rm)' --deny-tool='shell(git push)' --deny-tool='shell(git reset)' --deny-tool='shell(git clean)' --deny-tool='shell(git checkout)'`. If the caller supplies `--credits <N>`, replace 10 with N. If the caller marks the task mechanical, add `--effort low`; if the caller supplies `--effort <level>`, forward that level. If the caller's prompt states the target repo has a `.github/agents/mechanical-worker.agent.md` profile, add `--agent=mechanical-worker` too — it's a coarser convenience layer (tool categories + a default model + behavioral guardrails), not a replacement for the `--deny-tool` flags above, which remain the actual safety net since the agent profile's `tools` field can't express git subcommand-level denial.
...
- A task that explicitly needs a tool outside git/write (for example npm, dotnet, or python) may add the matching `--allow-tool='shell(<tool>:*)'`; never use `--allow-all`.
